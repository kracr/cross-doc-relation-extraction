...
wandb.init(project='closedsetting', entity='xxxx',name='context_added_sentence_explanation')
...
def process(tokenizer, h, t, doc0, doc1,ke):  
    ...
    rele["RELE2"]=pbuf_ht
    rele["RELE1"]=nbuf_b
    with open("relevance.txt","a+") as f:
        print(rele,file=f)
    for i in range(n0):
...
def collate_fn(batch, args, relation2id, tokenizer, redisd, encoder, sbert_wk):
    ...
    if batch[0][-1] == 'o':
        ...
        #contextdict={}
        #contextdict["key"]=batch[0]
        #contextdict["head"]=h
        #contextdict["tail"]=t
        #contextdict["label"]=batch[1]
        #print("RELATION:",r)
        dps = batch[2]
        if len(dps) > 8:
        ...
        decoded_ids = tokenizer.decode(selected_ids.tolist()[0])  # Decoding the token IDs to string
        contextdict={}
        contextdict["key"]=batch[0]
        contextdict["head"]=h
        contextdict["tail"]=t
        contextdict["label"]=batch[1]
        contextdict["explaination"]=decoded_ids
        
        #print(contextdict)

        with open("/home/ecrim/ecrim/explanation.txt","a+") as df:
            print(contextdict,file=df)
        ...
        for idx, example in enumerate(examples):
            ...
            contextdict["head"]=h_name
            contextdict["tail"]=t_name
            contextdict["label"]=df
